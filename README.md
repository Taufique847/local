...
- **The calendar is day-only.** Times and conflicts are now correct — slots are generated in the business's timezone, every offered slot is one the booking path will accept, and conflicts are checked against the assigned technician rather than the whole business, so a crew can hold concurrent jobs. What is still missing is surface area: the backend range endpoint for week and month views exists and is never called, there is no drag-and-drop, and there are no recurring appointments.
...
- **Tests cover the dangerous paths and the money paths, not the product.** 546 tests over tenancy, webhook signatures, RBAC, auth token lifecycles, pricing and job completion, notifications, templates, equipment and property data, segment campaigns and email consent. Every guard they protect was confirmed by deliberately breaking it: 294 mutations attempted, 291 caught, and the two survivors are documented in place as behaviour-neutral. The frontend and the voice pipeline have no automated coverage.
