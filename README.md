...
- **The calendar has no drag-and-drop and no recurring appointments.** What it does have is correct: day, week and month views all rendered in the business's timezone, every offered slot is one the booking path will accept, and conflicts are checked against the assigned technician rather than the whole business, so a crew can hold concurrent jobs. Technician lanes in the day view are also not built — they are worth having once jobs are routinely assigned, which needs the technician picker.
...
- **Tests cover the dangerous paths and the money paths, not the product.** 558 tests over tenancy, webhook signatures, RBAC, auth token lifecycles, pricing and job completion, notifications, templates, equipment and property data, segment campaigns and email consent. Every guard they protect was confirmed by deliberately breaking it: 307 mutations attempted, 304 caught, and the two survivors are documented in place as behaviour-neutral. The frontend and the voice pipeline have no automated coverage.
