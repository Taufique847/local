...
| **Segments & campaigns** | Saved customer segments with live (never cached) counts. Filters on tags in/all/none, lifetime value, last service date, never-serviced, property type and equipment brand. Campaigns send per recipient through `NotificationService`, so quiet hours and the SMS opt-out apply; campaign texts are refused without an opt-out notice, and the audience is capped at 500. **See the gap below about email consent before using the email channel** |
...
- **Tests cover the dangerous paths and the money paths, not the product.** 377 tests over tenancy, webhook signatures, RBAC, auth token lifecycles, job-completion pricing, notifications, templates, equipment and property data, and segment campaigns. Every guard they protect was confirmed by deliberately breaking it: 141 mutations attempted, 139 caught, and both survivors are documented in place as behaviour-neutral. The frontend and the voice pipeline have no automated coverage.
- **There is no email-consent flag, so email campaigns have no unsubscribe.** `Customer.isOptedOut` is set only by an SMS `STOP` keyword and deliberately does not suppress email — a customer who stopped texts still needs their own invoice, and transactional email is exempt from CAN-SPAM opt-out. A **campaign** is not exempt. The segment campaign feature will happily send on the email channel with no way for the recipient to opt out, and nothing records that they asked to. Either add the flag or keep campaigns on SMS only, where the opt-out is enforced.
