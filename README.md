...
- **The calendar has no recurring appointments and no technician lanes.** What it does have is correct: day, week and month views rendered in the business's timezone, drag-and-drop reschedule that goes through the locked path so opening hours and the per-technician conflict check still apply, every offered slot is one the booking path will accept, and conflicts checked against the assigned technician rather than the whole business so a crew can hold concurrent jobs. Recurrence is the piece a maintenance-plan feature would need; lanes are worth having once jobs are routinely assigned, which needs the technician picker.
...
- **Tests cover the dangerous paths and the money paths, not the product.** 568 tests over tenancy, webhook signatures, RBAC, auth token lifecycles, pricing and job completion, notifications, templates, equipment and property data, segment campaigns and email consent. Every guard they protect was confirmed by deliberately breaking it: 330 mutations attempted, 327 caught, and the two survivors are documented in place as behaviour-neutral. The frontend and the voice pipeline have no automated coverage.
